...
class ImageFeeder():
    def __init__(self):
        ...
        self.img_sub = rospy.Subscriber(self.cam_topic, Image, self.cameraCallback)
        # model objects
        self.model = load_model(os.path.join(os.path.dirname(__file__), "cnn_model.h5"))
        print('LOADED MODEL')

    def cameraCallback(self,cam_msg):
        # can return header, height, width, encoding (rgb8), step, data
        try:
            img = self.bridge_object.imgmsg_to_cv2(cam_msg, desired_encoding='bgr8')
            print('LOADED IMAGE')
        ...
        cropped_img = img[200:800,700:1300]
